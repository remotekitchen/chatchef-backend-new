...
# === SYNC FUNCTION ===
def sync_Do_record_to_lark(record, field_info):
    token = get_lark_token()
...
# === BULK SYNC FUNCTION ===
def push_DO_invoices_to_lark_from_history(start_date, end_date):
    ...
    records = PayoutHistory.objects.filter(
        statement_start_date=start_date,
        statement_end_date=end_date
    )
    print(f"🗂️ Found {records.count()} records to sync from DO for: {start_date.date()} to {end_date.date()}")
    ...
    for payout in records:
        ...
        orders = payout.orders.all()
        print(f"🧾 Processing payout ID {payout.id} — {orders.count()} orders")
        for order in orders:
            ...
            if order.payment_method == Order.PaymentMethod.STRIPE:
                stripe_fees = float("{0:.2f}".format((order.total * 2.9 / 100) + 0.30))
                if restaurant.service_fee:
            ...
            formatted = {
                ...
                "Bag fees": float(order.bag_price or 0),
                "Customer absorb on delivery fees": customer_absorb_delivery_fees,
                "Delivery fees expense": delivery_expense,
                "Discount": float(order.discount or 0),
                "Item Price": float(order.subtotal - order.refund_amount) if order.refund_reason else float(order.subtotal or 0),
                "Order Mode": validate_select_value("Order Mode", order.order_method, field_info),
                "Original Delivery Fees": float(order.original_delivery_fee or 0),
                "Payment Type": validate_select_value("Payment Type", order.payment_method, field_info),
                "Selling price (inclusive of tax)": float(order.total - order.refund_amount) if order.refund_reason else float(order.total or 0),
                "Service fee to chatchefs": service_fee,
                "Service fees to restaurant": service_fees_to_res,
                "Stripe Fees": stripe_fees,
                "Sub-Total Payment": sub_total,
                "Tax": float(order.tax or 0),
                "Tips for restaurant": float(order.tips_for_restaurant or 0),
                "Utensil fees": float(order.utensil_price or 0),
            }

            print("📦 Syncing record:", formatted["Order ID"])
            sync_Do_record_to_lark(formatted, field_info)
            print("✅ Finished syncing", formatted["Order ID"])
